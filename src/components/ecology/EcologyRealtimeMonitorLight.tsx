  ...
  return (
    <div className="bg-white rounded-lg p-3 border border-gray-200 shadow-sm">
      <div className="flex items-center justify-between mb-2">
        <h4 className="text-gray-700 text-xs font-medium">{title}</h4>
      </div>
      <div className="relative" style={{ height }}>
        ...
        >
          <defs>
            <linearGradient id={`gradient-light-${title}`} x1="0%" y1="0%" x2="0%" y2="100%">
...
function MapPanel({ 
  ...
  return (
    <div className={`relative rounded-xl overflow-hidden border border-gray-200 shadow-sm ${
      isFullscreen ? 'fixed inset-4 z-50 bg-white' : 'h-full'
    }`}>
      {/* 地图背景 */}
      <img
        src="/地图-生态环境用.png"
      ...
      {/* 渐变遮罩 */}
      <div className="absolute inset-0 bg-gradient-to-b from-blue-50/30 via-transparent to-blue-100/50" />
      
      {/* 标题和全屏按钮 */}
      <div className="absolute top-3 left-3 right-3 flex items-center justify-between z-20">
        <div className="bg-white/90 backdrop-blur-sm rounded-lg px-3 py-1.5 border border-gray-200 shadow-sm">
          <span className="text-gray-700 text-sm font-medium">监测设备分布</span>
        </div>
        <button
          onClick={onToggleFullscreen}
          className="bg-white/90 hover:bg-gray-100 backdrop-blur-sm rounded-lg p-2 border border-gray-200 shadow-sm transition-colors"
        >
          {isFullscreen ? (
          ...
          >
            ...
            <div 
              className={`w-8 h-8 rounded-full border-2 flex items-center justify-center transition-all shadow-lg ${
                isSelected 
                  ? 'ring-4 ring-blue-200 border-white' 
                  : isCurrentTheme 
                    ? 'border-white' 
                    : 'border-white/60'
              }`}
              style={{ 
                ...
                boxShadow: isSelected 
                  ? `0 0 20px ${point.themeColor}60` 
                  : '0 2px 8px rgba(0,0,0,0.15)',
                opacity: isCurrentTheme ? 1 : 0.5
              }}
            >
            ...
            {/* 悬浮提示 */}
            <div className="absolute top-10 left-1/2 -translate-x-1/2 whitespace-nowrap bg-white/95 backdrop-blur-sm px-3 py-2 rounded-lg shadow-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none border border-gray-200">
              <div className="text-gray-800 text-sm font-medium">{point.name}</div>
              <div className="text-gray-500 text-xs">{point.location}</div>
              <div className="flex items-center gap-1 mt-1">
                <div className={`w-2 h-2 rounded-full ${point.status === 'online' ? 'bg-green-500' : 'bg-gray-400'}`} />
                <span className="text-xs text-gray-600">{point.status === 'online' ? '在线' : '离线'}</span>
              </div>
            </div>
          </div>
        )
      ...
      {/* 图例 */}
      <div className="absolute bottom-3 left-3 bg-white/95 backdrop-blur-sm rounded-xl p-3 border border-gray-200 shadow-sm z-20">
        <div className="text-gray-700 text-xs font-medium mb-2">设备图例</div>
            ...
            return (
              <div key={t.id} className="flex items-center justify-between gap-4 text-xs">
                ...
                <div className="flex items-center gap-1">
                  <span className="text-green-600 font-medium">{stats[t.id as keyof typeof stats].online}</span>
                  <span className="text-gray-400">/</span>
                  <span className="text-gray-600">{stats[t.id as keyof typeof stats].total}</span>
                </div>
              </div>
            )
          })}
...
// 主组件 - 白天模式
export function EcologyRealtimeMonitorLight() {
  ...
  return (
    <div className="h-full flex flex-col bg-gradient-to-br from-gray-50 via-blue-50/30 to-gray-100 overflow-hidden">
      {/* 顶部主题切换 - 美化样式 */}
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    boxShadow: isActive ? `0 2px 12px ${theme.color}30` : 'none',
                  }}
                >
      ...
      <div className="flex-1 flex gap-3 p-3 overflow-hidden">
        ...
        <div className="w-[300px] flex-shrink-0 flex flex-col gap-3">
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-3">
            <div className="text-gray-500 text-xs mb-2">当前监测设备</div>
              ...
              >
                <div className="flex items-center gap-2">
                  <div 
                    ...
                    style={{ 
                      backgroundColor: selectedDevice?.status === 'online' ? currentTheme.color : '#9ca3af' 
                    }}
                  />
                  <span className="text-gray-800 text-sm">{selectedDevice?.name || '选择设备'}</span>
                </div>
                  ...
                  {currentDevices.map((device) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-gray-50 transition-colors ${
                        selectedDeviceId === device.id ? 'bg-gray-50' : ''
                      }`}
                    >
                      <div 
                        className="w-2 h-2 rounded-full"
                        style={{ backgroundColor: device.status === 'online' ? currentTheme.color : '#9ca3af' }}
                      />
                      <div className="flex-1">
                        <div className="text-gray-800 text-sm">{device.name}</div>
          ...
          {/* 监测指标卡片 - 一行2个 */}
          <div className="flex-1 bg-white/80 rounded-xl border border-gray-200 shadow-sm p-3 overflow-y-auto">
            <div className="grid grid-cols-2 gap-2">
              {currentFactors.map((factor) => {
                ...
                return (
                  <div 
                    key={factor.key}
                    className="bg-white rounded-lg p-3 border border-gray-100 shadow-sm hover:shadow transition-all"
                    style={{ borderTopColor: factor.color, borderTopWidth: '3px' }}
                  >
                    <div className="flex items-center gap-1.5 mb-1.5">
                      ...
                      <span className="text-gray-500 text-xs truncate">{factor.name}</span>
                    </div>
                    <div className="flex items-baseline gap-0.5">
                      <span className="text-lg font-bold text-gray-800">{displayValue}</span>
                      <span className="text-gray-400 text-xs">{factor.unit}</span>
                    </div>
                  </div>
                )
              })}
        ...
        {/* 右侧：统计图区域 */}
        <div className="w-[320px] flex-shrink-0 bg-white/80 rounded-xl border border-gray-200 shadow-sm p-3 overflow-y-auto">
          <div className="text-gray-700 text-sm font-medium mb-3">趋势变化</div>
...
export default EcologyRealtimeMonitorLight
